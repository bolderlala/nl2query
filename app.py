...
5. Use the SQL results to understand the correct answer. For most databases, translate the SQL logic faithfully. Exception: Key-Value stores have no secondary indexes, so they must fetch all entities and leave filtering to the client — do NOT use SQL result IDs to cherry-pick KV keys.
...
- Document Database (MontyDB): Output a Python expression starting with db["collection"].find({filter}). Use MongoDB operators ($gt, $gte, $lt, $lte, $eq, $ne, $in, $regex, $and, $or, $elemMatch). For sorting add .sort("field", 1 or -1). IMPORTANT: MontyDB does NOT support aggregate(), projection operators ($), or dot-notation in sort paths. NEVER use .sort("enrollments.score", ...) or any nested path — it sorts on the wrong array element. Only use .sort() on top-level fields like "name", "gpa", "year". For queries about nested enrollment data, use $elemMatch to filter and return the full student documents — the user will see all enrollment details. For "highest/lowest score" questions, just find all matching students with $elemMatch; do NOT attempt to sort or limit by nested scores. When filtering the same field multiple times (e.g., two $elemMatch on "enrollments"), you MUST use $and: [{"enrollments": {"$elemMatch": ...}}, {"enrollments": {"$elemMatch": ...}}]. Never repeat the same key in a Python dict literal.
- Key-Value Store (Redis): Output one Redis command per line. Available commands: GET, HGETALL, HGET, SMEMBERS, KEYS, ZRANGEBYSCORE, ZRANGE, ZREVRANGE. Keys follow patterns: student:{id}, course:{id}, enrollment:{sid}:{cid}, student:{id}:courses, course:{id}:students, scores:{course_id}. IMPORTANT: Key-value stores have NO secondary indexes — you CANNOT filter by attributes like GPA, major, or bio. If the SQL query filters on an attribute (e.g., WHERE gpa > 3.7 or WHERE bio LIKE '%...%'), the KV query MUST fetch ALL relevant entities (HGETALL student:1 through HGETALL student:8 for students, HGETALL course:101 through HGETALL course:105 for courses) because KV has no way to know which ones match without checking each one. The only exception is scores:{course_id} sorted sets, which CAN filter by score range using ZRANGEBYSCORE. For relationship lookups (e.g., "courses for student 1"), use SMEMBERS student:1:courses. Always return the data, not just key names.
...
- Vector Database (ChromaDB): Output a Python expression: client.get_collection("collection_name").query(...). Collections: student_profiles (has student bios), course_catalog (has course descriptions). ALWAYS use .query() with query_texts — NEVER use .get(). Use query_texts for semantic search, n_results for limit. IMPORTANT: Vector search finds by meaning, not exact values. Always rely on query_texts for the main search. Only add a where filter for simple exact string metadata matches (e.g., major or department). NEVER filter on numeric fields (scores, GPA, year) in where — ChromaDB where clauses do not support numeric comparisons reliably. Instead, describe the intent in query_texts and let semantic search do the work. Set n_results high enough to capture all relevant matches from the SQL results."""
